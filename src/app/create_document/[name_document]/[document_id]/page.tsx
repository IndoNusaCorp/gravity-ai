...
import { Search, Send, X, Plus, Minus, FileText, BookOpen, GraduationCap, Newspaper, Check, PenTool, Link as LinkIcon, Upload, Edit3, Calculator, AlignLeft, MessageSquare, Table, Network, MoreHorizontal, Save } from "lucide-react";
import { useState, useRef, useEffect, useCallback } from "react";
import Markdown from "markdown-to-jsx";
import { Analytics } from "@vercel/analytics/next"
import { AutoSave, RestoreAutoSave, type UploadedImage } from "@/components/autosave";
import { useParams, useRouter } from 'next/navigation'
import { LastDocument, Authentication } from '@/firebase/firebase.configuration';
import { doc, setDoc } from 'firebase/firestore';

export default function NewDocument() {
  //state untuk useparams
  //supaya jadi /create_document/[name_document]/[document_id]
  const param = useParams<{ name_document: string; document_id: string }>();
  // Nama dokumen ikut di URL, jadi harus di-decode dulu (spasi jadi %20 dst)
  const name_document = decodeURIComponent(param.name_document ?? "");
  const document_id = param.document_id;
  ...
  //state untuk memberikan nama dokumen (name_document)
  const [NameDocument, setNameDocument] = useState("");

  //state untuk fitur simpan (Last Document)
  const [savetoLastDocument, setsavetoLastDocument] = useState("");

  //logika untuk fitur simpan (last document)
  const handleSavetoLastDocument = async () => {
    // Penjaganya dipindah ke document_id: itu yang benar-benar wajib ada untuk
    // menyimpan. State savetoLastDocument sekarang dipakai mencatat statusnya.
    if (document_id) {
      const database = LastDocument;

      // Aturan Firestore mensyaratkan user sudah masuk
      const user = Authentication.currentUser;
      if (!user) {
        setsavetoLastDocument("gagal");
        console.log("Gagal menyimpan dokumen ke sistem GravityAI: belum masuk akun");
        return;
      }

      // Ambil naskah tiap halaman, sama caranya seperti AutoSave ke localStorage
      ...
      try {
        // Alamatnya: create_document/last_document/<uid>/<document_id>
        // uid dipakai sebagai pemisah supaya naskah antar akun tidak tercampur.
        await setDoc(
          doc(database, "create_document", "last_document", user.uid, document_id),
          {
            name: name_document,
        ...
        setsavetoLastDocument("tersimpan");
        setToast({ message: "Dokumen tersimpan", visible: true });
        setTimeout(() => setToast({ message: "", visible: false }), 3000);
        console.log("Berhasil menyimpan dokumen ke sistem GravityAI");
      } catch (error) {
        setsavetoLastDocument("gagal");
        setToast({ message: "Gagal menyimpan dokumen", visible: true });
        setTimeout(() => setToast({ message: "", visible: false }), 3000);
        console.log("Gagal menyimpan dokumen ke sistem GravityAI", error);
      }
    } else {
      setsavetoLastDocument("gagal");
      console.log("Gagal menyimpan dokumen ke sistem GravityAI")
    }
  }

  //logika untuk membuat nama dokumen
  const handleNameDocument = () => {
    if (NameDocument.trim()) {
      // name_document berasal dari URL, jadi tidak bisa di-set lewat state.
      // Ganti namanya = pindah URL, id dokumen tetap sama.
      // encodeURIComponent supaya spasi & karakter aneh tidak merusak alamat.
      const slug = encodeURIComponent(NameDocument.trim());
      route.replace(`/create_document/${slug}/${document_id}`);

      console.log("Buat nama dokumen berhasil");
    } else {
      console.log("Gagal membuat nama dokumen");
    }
  }

  // Modal nama dokumen: hanya muncul untuk dokumen yang baru dibuat,
  // yaitu selama slug di URL masih "untitled" bawaan /create_document.
  const [isNameModalOpen, setIsNameModalOpen] = useState(false);

  useEffect(() => {
    if (name_document === "untitled") setIsNameModalOpen(true);
  }, [name_document]);
  ...
  const scheduleAutoSave = useCallback(() => {
    ...
    autoSaveTimerRef.current = AutoSave(document_id, name_document, pageNumberRef.current, uploadedImagesRef.current);
  }, [document_id, name_document]);

  // Effect: baca naskah tersimpan dari localStorage saat halaman pertama kali dibuka
  useEffect(() => {
    const savedDocument = RestoreAutoSave(document_id);
  ...
  return (
    <>
      <div className="relative flex flex-col items-center min-h-screen bg-[#D9E4D1]/50 dark:bg-[#0D0606]/50 font-sans overflow-x-hidden py-12 transition-colors duration-300">
        ...
        {/* Card name and card save */}
        <div data-export-ignore="true" className="hidden xl:flex fixed left-4 top-4 items-center gap-2 z-40">

          {/* Card name document */}
          <div className="flex h-16 items-center px-2 max-w-[11rem] bg-white/40 dark:bg-[#0D0606]/40 backdrop-blur-xl border border-white/40 dark:border-white/10 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)]">
            <button
              onClick={() => {
                // Kosongkan dulu kalau masih bawaan, supaya user tidak perlu hapus "untitled"
                setNameDocument(name_document === "untitled" ? "" : name_document);
                setIsNameModalOpen(true);
              }}
              title="Ganti nama dokumen"
              className="group flex items-center gap-2 w-full px-2.5 py-1.5 rounded-xl hover:bg-[#0D0606]/5 dark:hover:bg-[#D9E4D1]/10 transition-colors"
            >
              <FileText className="w-4 h-4 shrink-0 text-[#0D0606]/50 dark:text-[#D9E4D1]/50" />
              <span className={`truncate text-sm font-semibold ${name_document === "untitled"
                ? 'text-[#0D0606]/40 dark:text-[#D9E4D1]/40 italic'
                : 'text-[#0D0606] dark:text-[#D9E4D1]'
                }`}>
                {name_document === "untitled" ? "Tanpa judul" : name_document}
              </span>
              <Edit3 className="w-3.5 h-3.5 shrink-0 text-[#0D0606]/0 dark:text-[#D9E4D1]/0 group-hover:text-[#0D0606]/50 dark:group-hover:text-[#D9E4D1]/50 transition-colors" />
            </button>
          </div>

          {/* Card Save */}
          <div className="flex h-16 items-center px-2 bg-white/40 dark:bg-[#0D0606]/40 backdrop-blur-xl border border-white/40 dark:border-white/10 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)]">
            <button onClick={handleSavetoLastDocument}
              type="button"
              title="Simpan dokumen"
              className="group flex items-center gap-2 px-2.5 py-1.5 rounded-xl hover:bg-[#0D0606]/5 dark:hover:bg-[#D9E4D1]/10 transition-colors active:scale-95"
            >
              <Save className="w-4 h-4 shrink-0 text-[#0D0606]/50 dark:text-[#D9E4D1]/50 group-hover:text-[#0D0606] dark:group-hover:text-[#D9E4D1] transition-colors" />
        ...
        <main className="relative z-10 w-full flex flex-col items-center justify-start sm:p-4 transition-all duration-500 min-h-[85vh] md:pl-[23rem] md:pr-72 pt-20">
          ...
          {/* Judul dokumen versi layar sempit — kartu kiri atas tidak muat di sini */}
          <div data-export-ignore="true" className="xl:hidden w-[calc(100%-2rem)] sm:w-full max-w-[var(--paper-max-width,794px)] mb-4">
            <button
              onClick={() => {
                setNameDocument(name_document === "untitled" ? "" : name_document);
                setIsNameModalOpen(true);
              ...
              title="Ganti nama dokumen"
              className="group flex items-center gap-2 px-3 py-1.5 -ml-1 rounded-lg hover:bg-[#0D0606]/5 dark:hover:bg-[#D9E4D1]/10 transition-colors"
            >
              <span className={`text-base font-semibold ${name_document === "untitled"
                ? 'text-[#0D0606]/40 dark:text-[#D9E4D1]/40 italic'
                : 'text-[#0D0606] dark:text-[#D9E4D1]'
                }`}>
                {name_document === "untitled" ? "Tanpa judul" : name_document}
              </span>
              <Edit3 className="w-3.5 h-3.5 text-[#0D0606]/0 dark:text-[#D9E4D1]/0 group-hover:text-[#0D0606]/50 dark:group-hover:text-[#D9E4D1]/50 transition-colors" />
            </button>
          </div>
      ...
      {/* Modal beri nama dokumen — muncul sekali saat dokumen baru dibuat */}
      <AnimatePresence>
        {isNameModalOpen && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                <div className="p-2.5 bg-[#0D0606]/10 dark:bg-[#D9E4D1]/10 rounded-xl">
                  <FileText className="w-5 h-5 text-[#0D0606] dark:text-[#D9E4D1]" />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-[#0D0606] dark:text-[#D9E4D1]">Beri nama dokumen</h3>
                  <p className="text-xs text-[#0D0606]/70 dark:text-[#D9E4D1]/70">Nama ini akan tampil di alamat dokumenmu</p>
                </div>
              </div>
              ...
              <input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter' && NameDocument.trim()) handleNameDocument(); }}
                placeholder="Contoh: Skripsi Bab 1"
                className="w-full bg-zinc-50 dark:bg-zinc-800 border-2 border-[#0D0606]/20 dark:border-[#D9E4D1]/20 focus:border-zinc-900 dark:focus:border-white rounded-xl px-4 py-3 text-sm text-[#0D0606] dark:text-[#D9E4D1] placeholder:text-[#0D0606]/50 outline-none transition-colors"
                autoFocus
              />
              ...
              <div className="flex items-center gap-3 mt-6">
                ...
                <button
                  onClick={handleNameDocument}
                  disabled={!NameDocument.trim()}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-[#0D0606] dark:bg-[#D9E4D1] text-[#D9E4D1] dark:text-[#0D0606] rounded-xl text-sm font-semibold hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors disabled:opacity-40 disabled:cursor-not-allowed active:scale-95"
                >
